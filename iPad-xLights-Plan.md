...
|---|---|---|---|
| ~~`DMX_ChannelsNotebook` per-channel VC~~ | ~~DMX~~ | **landed** — each of the 48 `DMXChannelRow`s now renders a `ValueCurveButton` between the editable value field and the invert toggle, keyed against `E_VALUECURVE_DMX<N>` via a synthesised PropertyMetadata (id `DMX<N>`, range 0..255). Base slider + numeric field dim + disable while the curve is active, matching the pattern used by every other VC-capable slider | — |
...
| ~~`Video_DurationRow`~~ | ~~Video~~ | **landed** — probes the picked file's duration via `AVURLAsset.load(.duration)` on a `.task(id: path)` that re-fires whenever the filename setting changes, displaying `mm:ss.ff`. "Match Effect to Video Duration" button sets the effect's end time to `start + duration` via `resizeEffectEdge`. Relative paths resolve against the show folder before `AVAsset` opens them; errors (path unresolvable, file can't be opened, zero/infinite duration) show an inline orange hint | — |
...
| ~~`SubBuffer` x/y center offsets~~ | ~~Buffer panel~~ | **landed** — `xc` / `yc` slots added to the `Slot` enum, rendered as a third "X Centre / Y Centre" row via the generic `axisField` helper with a ±100 range (matches `PixelBuffer.cpp:1964-1981`). Center values shift the whole rect at render time; the canvas doesn't yet visualise them but the values commit correctly | — |
| ~~`SubBuffer` VC per slot~~ | ~~Buffer panel~~ | **landed** — new `SubBufferSlotVCButton` per slot opens the full `ValueCurveEditorSheet`. `EditableValueCurve` + the sheet gained an optional `customPersist` closure so the VC string writes inline into the `B_CUSTOM_SubBuffer` composite instead of a dedicated `<prefix>VALUECURVE_<id>` key. Toggling the curve off collapses the slot back to a plain float (last-known value or the slot's natural default) so round-tripping doesn't persist a dead `Active=FALSE` marker inside the composite string | — |
| ~~`Sparkles` VC button~~ | ~~Color panel~~ | **landed** — `ValueCurveButton` inline next to the frequency slider via a synthesised PropertyMetadata (id `SparkleFrequency`, prefix `C_`, range 0..200); base slider dims + disables while the curve is active, same pattern as every other VC-capable slider | — |
| ~~Blending Adjust/Reverse runtime disable~~ | ~~Blending~~ | **landed** — new `runtimeDisabled` hook in `EffectPropertyView` checks each transition-side Adjust / Reverse property against the sibling Fade + Type settings and applies `.disabled()` + 0.4 opacity to the slider / checkbox / editable value field. Matches the compound `fade==0 OR type ∈ set` condition in `BlendingPanel::ValidateWindow` without needing a schema extension | — |
| ~~ChromaKey sensitivity range~~ | ~~Color panel~~ | **landed** — corrected iPad range from 0..100 to desktop's actual 1..255 (default 1). `ColorPanel.cpp:374` confirms the slider feeds the raw distance threshold to `ColourDistance` without further scaling | — |
...
| ~~Inspector header "time" row~~ | ~~Effect~~ | **landed** — `timingRow` now pulls the selected effect's start / end from `rows[…].effects[…]` instead of the captured `selectedEffect` snapshot (so drag / resize / undo updates reflect live). Added a third column between Start and End that shows `▶ <mm:ss.ff>` during the selection-scoped scrub loop, tracking `playPositionMS` | — |
