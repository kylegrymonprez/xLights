...
migration), C (effect settings inspector), and D (Model Preview
polish) are complete and have no pending items. Phase B has the
Metal-backed grid, basic selection, drag / resize, and long-press
menu shipped, with the remaining parity work tracked in
...
Moving Head / DMX) all landed; two polish follow-ups (MH colour /
path authoring and DMX state persistence) live in `followups.md`.
Cross-phase odds and ends also live in `followups.md`.
