...
**Not started.** B97 Find / Replace landed inside the grid;
everything else here is desktop-only.
...
| AP-9 | BufferPanel — verify gap vs Phase C inspector Buffer tab | P2 | M (verify) |
...
| AP-12 | ValueCurvesPanel — verify gap vs iPad's `XLValueCurve` | P2 | M (verify) |
| AP-13 | BlendingPanel — verify gap vs iPad's BlendingPanelViews | P2 | M (verify) |
