...
bool xLightsXmlFile::AddAlternateMedia(const wxString& ShowDir, const wxString& filename )
{
    ...
    //Check to see if this file is already there....
    for (size_t i = 0; i < alternateMedia.size(); ++i )
    {
        if( alternateMedia[i] == media_file )
        {
            logger_base.debug("alternateMedia file already exists");
            return false;
        }
    }
    ...
    //Maybe need pieces of this for validation @@@
    //ObtainAccessToURL(filename.ToStdString());
//    if ((filename != wxEmptyString) && FileExists(filename) && wxIsReadable(filename)) {
//        logger_base.debug("SetMediaFile: Creating audio manager");
//        audio = new AudioManager(std::string(filename.c_str()), GetFrameMS());
...
void xLightsXmlFile::CreateNew()
{
    ...
    AddChildXmlNode(node, "alternateMedia", wxEmptyString);
...
wxArrayString xLightsXmlFile::GetAlternateMedia()
{
